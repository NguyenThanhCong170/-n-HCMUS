...
const init = () => {
    ...
    // Xử lý mic để thành chữ 
    document.addEventListener("mic:text", async (e) => {
        const raw = e.detail?.text;

        // Lấy 5 ký tự đầu 
        const word = raw.toUpperCase().slice(0, 6);
        while (currentWord.length) onkeydown("Backspace");
        console.log(word);
        await AutoType(word);
    });
